...
describe("buildCommentDeepLink", () => {
  it("builds a comment deeplink URL with a path-style root/file and a comment_id query param", () => {
    const got = buildCommentDeepLink("http://localhost:15174", "code", "foo.md", "c-001");
    expect(got).toBe("http://localhost:15174/code/foo.md?comment_id=c-001");
  });
  ...
  it("escapes special characters in root, path and commentId", () => {
    ...
    expect(got).toContain("/%E3%83%AC%E3%83%93%E3%83%A5%E3%83%BC/");
    expect(got).toContain("/%E6%97%A5%E6%9C%AC%E8%AA%9E%2Fnote%20v2.md");
