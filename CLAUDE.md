...
```markdown
<!-- @comment id="c-1715414400000" author="kishira" date="2026-05-11" target="この段落を直したい" -->コメント本文<!-- /@comment -->
```

### 構造

- 開始タグ: `<!-- @comment <属性...> -->`
- コメント本文: 開始タグと終了タグの間の任意テキスト
- 終了タグ: `<!-- /@comment -->`
...
| `target` | yes | コメントが付いた本文テキスト（コメント開始タグの直前に出現するテキスト） |
...
2. **`target` 属性が付いた範囲がレビュー対象。** コメント本文は `target` に対する要望／指摘である。
...
<!-- @comment id="c-001" author="kishira" date="2026-05-20" target="JWT で認証する" -->JWT じゃなくて Session ベースに統一したい<!-- /@comment -->
...
→ `target` の文言（"JWT で認証する"）をコメント本文の要望（"Session ベースに統一"）に従って書き換え、コメントブロックは削除する。
