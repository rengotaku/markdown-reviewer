...
>
> **行頭のブロック記号（`>` / `-` `*` `+` / `1.` `1)` / `#`）は付けても付けなくてもよい**。
> サーバは生 Markdown の行に対して照合するので記号込みでも一致し、Web UI は
> 照合時に行頭の記号を読み飛ばす（記号は ProseMirror のブロックテキストには現れない）。
> ただし**付けない方が安全**。付けた場合、ハイライトの範囲は記号を除いた本文部分になる。
...
Web UI（Tiptap エディタ）は、本文中の Markdown リンク `[text](href)` の `href` を、開いている
ファイルの root-relative パスを基準に解決し、**同一レビュールート配下の他ファイルを指すリンク**
だけを「内部リンク」として扱う。内部リンクはクリックで対象ファイルを別タブとして開き、ホバー
すると内容プレビューを表示する。それ以外の `href` はアイコン付きで新規タブに開く外部リンクと
して扱われ、`#heading` のようなページ内アンカーは何もしない。

解決規則:

- `./` / `../` から始まる相対パスは、リンクを書いたファイル自身の場所を基準に解決する
- `/` から始まるパスはレビュールートの先頭からの絶対パスとして解決する
- `?query` と `#fragment` は解決前に切り落とす（対象ファイルの特定にのみ使う）
- パーセントエンコード（`%20` 等）はデコードしてから解決する

次の場合は内部リンクにならず、外部リンク扱いになる:

- スキーム付き URL（`https:`, `mailto:` 等）
- プロトコル相対 URL（`//host/...`）
- `#` のみで始まるページ内アンカー
- `..` でレビュールートの外に出るパス

**AI 向けの推奨**: 同一ルート配下の関連ファイルに触れるときは、ファイル名を文字列で書くのでは
なく相対リンク（`[text](./path.md)` 等）で書く。Web UI 上でクリック遷移・ホバープレビューが
効くようになる。
