...
別管理（sidecar: `~/.config/reviewer/<root>/<path>/review.json`）に保持される。
...
```json
{
  ...
  "comments": [
    { "id": "c-001", "scope": "inline", "author": "kishira", "body": "36 時間では？",
...
status を更新する。ボディ: `{ "status": "resolved" }`（`open`/`resolved` のみ）。
...
正典本文にレビューマーカーは無い。AI は `GET /api/review/*path` で open コメントを整形済みで読み、
本文を `PUT /api/files/*path`（in-place）で更新し、`POST /api/replies` / `PATCH ...?status=resolved`
で対応状況を返す。`.vN.md` 等の派生ファイルは作らない。
